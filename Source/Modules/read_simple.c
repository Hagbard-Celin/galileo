...
static char *version="$VER: read.gfmmodule 0.1 "__AMIGADATE__" ";
	...
	register __d1 ULONG mod_data)
{
	...
	if (!first)
	{
		if (GetVar("galileo_read",data->title,sizeof(data->title),GVF_GLOBAL_ONLY)>0)
		{
			char *ptr=data->title;
						...
						while (item=ItemAddress(data->window->MenuStrip,(USHORT)msg_copy.Code))
						{
							...
							switch (itemid)
							{
								...
								case MENU_SAVE:
								{
									...
									// Set variable
									if (SetVar("galileo_read",buf,-1,GVF_GLOBAL_ONLY))
									{
										// Copy to ENVARC:
										SystemTagList("copy env:galileo_read envarc:",0);
									}

...
char *version="$VER: format.gfmmodule 0.1 "__AMIGADATE__" ";
				...
				switch (msg_copy.Class)
				{
					...
					case IDCMP_GADGETUP:
						switch (((struct Gadget *)msg_copy.IAddress)->GadgetID)
						{
							...
							// Long implies International and excludes Caching:
							case GAD_FORMAT_LONG:
								{
									BOOL state;

									// Get state
									state=GetGadgetValue(data->list,GAD_FORMAT_LONG);
									...
									// If on, check International
									if (state) SetGadgetValue(data->list,GAD_FORMAT_CACHING,0);
								}
								break;
...
BOOL format_open(format_data *data,BOOL noactive)
{
    ...
    // If long is on, disable caching
    if (data->default_long)
    {
		DisableObject(data->list,GAD_FORMAT_INTERNATIONAL,TRUE);
		DisableObject(data->list,GAD_FORMAT_CACHING,TRUE);
	}
...
BOOL start_format(format_data *data,unsigned short type,BOOL reopen)
{
	...
	if (data->default_long)
	{
		if (data->default_ffs) data->dos_type=ID_LONG_FFS_DISK;
		else data->dos_type=ID_LONG_DOS_DISK;
	}

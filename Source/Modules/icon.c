...
char *version="$VER: icon.gfmmodule 0.1 "__AMIGADATE__" ";
...
if	(data->prot_list = Att_NewList( 0 ))
	...
	for	(a = 0; a < 8; ++a)
		{
		// Create node
		if	(node = Att_NewNode( data->prot_list, GetString(locale,MSG_ICON_HOLD+a), a, 0 ))
			{
			// Set selection flag
...
// Bytes
if	(lock)
	ItoaU( data->fib.fib_Size, buf, data->decimal_sep );

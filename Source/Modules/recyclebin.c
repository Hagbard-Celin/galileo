...
static const char *version="$VER: recyclebin.gfmmodule 0.2 "__AMIGADATE__" ";
...
void recycle_bin_install(RecyclePacket *packet,recyclebin_data *data)
{
	...
	SetIconFlags(trash_icon,ICONF_BORDER_OFF);
...
void recyclebin_open_window(recyclebin_data *data)
{
	...
	// Try and find Galileo screen, if not get default
	if (!(screen=LockPubScreen("GALILEO.1")))

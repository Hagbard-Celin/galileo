	...
	register __d1 ULONG mod_data)
{
	...
	if (!run_once || run_once==2)
	{
		...
		if (GetVar("Galileo/Text Viewer",data->line_buffer,sizeof(data->line_buffer),GVF_GLOBAL_ONLY)>0)
		{
				...
				switch (msg_copy.Class)
				{
					...
					case IDCMP_GADGETUP:
							...
							data->appport)
						{
							...
							// No icon yet?
							if (!data->app_diskobj &&
								!(data->app_diskobj=GetDiskObject("PROGDIR:icons/read")) &&
						...
						while (item=ItemAddress(data->window->MenuStrip,(USHORT)msg_copy.Code))
						{
							...
							switch (itemid)
							{
								...
								case MENU_SAVE:
								{
									...
									// Set variable
									SetEnv("Galileo/Text Viewer",data->line_buffer,TRUE);

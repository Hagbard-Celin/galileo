...
Cfg_ButtonBank *convert_button_bank(struct galileogadgetbanks *,struct Config *,int);
	...
	register __a2 IPCData *owner_ipc)
{
	...
	if ((ReadOldConfig(name,&cstuff))==1)
	{
		...
		if (break_flag==1)
		{
			...
			{
				lsprintf(name,"PROGDIR:environment/%s",basename);
				success=1;
			}
	...
	char *basename)
{
	...
	if (convert&(1<<CONVERT_ENVIRONMENT))
	{
		...
		if (env=AllocVec(sizeof(CFG_ENVR),MEMF_CLEAR))
		{
			...
			// Build filename
			lsprintf(buffer,"PROGDIR:environment/%s",basename);

			// Try to open file to write
			while (iff=IFFOpen(buffer,MODE_NEWFILE,ID_GILO))
			{
				Cfg_Lister *lister;
				short len;
				...
				{
					lsprintf(buffer,"PROGDIR:buttons/%s_hotkeys",basename);
				...
				{
					lsprintf(buffer,"PROGDIR:buttons/%s_menu",basename);
				...
				// Bring the default button bank up by default
				{
					...
					// Write bank path
					lsprintf(buffer,"PROGDIR:buttons/%s",basename);
	...
	if (convert&(1<<CONVERT_BUTTONS))
	{
		...
		for (bank=cstuff->firstbank;bank;bank=bank->next,count++)
		{
			...
			{
				// Build name to save
				if (count==0) lsprintf(buffer,"PROGDIR:buttons/%s",basename);
				else lsprintf(buffer,"PROGDIR:buttons/%s.%ld",basename,count+1);
	...
	if (convert&(1<<CONVERT_DRIVES))
	{
		...
		if (bank=NewButtonBank(0,0))
		{
			...
			// Build name to save
			lsprintf(buffer,"PROGDIR:buttons/%s_drives",basename);
	...
	if (convert&(1<<CONVERT_FILETYPES))
	{
		...
		for (oldtype=cstuff->firsttype;oldtype;oldtype=oldtype->next)
		{
			...
			// Build filetype path
			lsprintf(buffer,"PROGDIR:filetypes/%s",oldtype->type);

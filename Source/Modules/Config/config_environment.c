	...
	register __d2 Att_List *script_list)
{
	...
	if (data->mode_list=Att_NewList(LISTF_POOL))
	{
		...
		if (pubscreen_list=LockPubScreenList())
		{
			...
			{
				// Check that this isn't Workbench or Galileo, and is public
				if (strcmp(pubscreen->psn_Node.ln_Name,"Workbench")!=0 &&
					strncmp(pubscreen->psn_Node.ln_Name,"GALILEO",7)!=0 &&
					...
					case IDCMP_GADGETDOWN:
						...
						switch (gadgetid)
						{
							...
							case GAD_ENVIRONMENT_OPTIONS_ICONPOS:
								...
								DisableObject(
									data->option_list,
									GAD_ENVIRONMENT_OPTIONS_REMOVE_GALILEO_IPOS,
									!msg_copy.Code);
								break;
			...
			if (data->icon_settings)
			{
				...
				for (node=(Att_Node *)data->icon_settings->list.lh_Head;node->node.ln_Succ;node=(Att_Node *)node->node.ln_Succ)
				{
					...
					if ((a==0 && !(data->config->desktop_flags&DESKTOPF_NO_CACHE)) ||
						...
						(a==7 && data->config->display_options&DISPOPTF_REMGALILEOPOS) ||
	...
	switch (option)
	{
		...
		case ENVIRONMENT_ICONS:
			...
			if (data->icon_settings)
			{
				...
				data->config->display_options&=~(DISPOPTF_ICON_POS|DISPOPTF_REMGALILEOPOS|DISPOPTF_REALTIME_SCROLL);
				...
				for (node=(Att_Node *)data->icon_settings->list.lh_Head;node->node.ln_Succ;node=(Att_Node *)node->node.ln_Succ)
				{
					...
					if (a==7 && node->node.lve_Flags&LVEF_SELECTED)
						data->config->display_options|=DISPOPTF_REMGALILEOPOS;
					else
...
void config_env_load(config_env_data *data,USHORT id)
{
		...
		id==BUTTONEDIT_MENU_DEFAULTS)
	{
		...
		if (id==BUTTONEDIT_MENU_NEW)
		{
			strcpy(path,"PROGDIR:environment/");
	...
	while (!ret)
	{
		...
		// Get filename
		if (ask && !(config_filereq(data->window,MSG_ENVIRONMENT_OPEN,path,"PROGDIR:environment/",0)))
			break;
...
BOOL config_env_save(config_env_data *data,short saveas)
{
	...
	while (ret)
	{
		...
		{
			// Get filename
			if (!config_filereq(data->window,MSG_ENVIRONMENT_SAVE,path,"PROGDIR:environment/",1))
				break;
		}

						...
						switch (gadgetid)
						{
							...
							case MENU_FUNCED_EXPORT_CMD:
								...
								if (WINREQUESTER(data->window))
								{
									...
									tags[4].ti_Data=(ULONG)"PROGDIR:Commands";
							...
							case MENU_FUNCED_COPY:
								{
									...
									// Open clipboard
									if (iff=IFFOpen((char *)0,IFF_CLIP_WRITE,ID_GILO))
									{
										// Stop editing
							...
							case MENU_FUNCED_PASTE:
								{
									...
									if (gadgetid==MENU_FUNCED_PASTE)
									{
										...
										// Open clipboard
										if (iff=IFFOpen((char *)0,IFF_CLIP_READ,ID_GILO))
										{
											// Find function
...
BOOL funced_command_req(FuncEdData *data,char *buffer,short type)
{
	...
	if (type>INST_COMMAND)
	{
		...
		tags[3].ti_Data=(ULONG)((type==INST_SCRIPT)?"s:":((type==INST_AREXX)?"PROGDIR:ARexx/":""));

	...
	register __a3 char *name)
{
	...
	// Get saved position
	if (LoadPos("Galileo/windows/filetypes",&pos,&fontsize))
	{
	...
	if (data->window)
	{
		...
		SavePos("Galileo/windows/filetypes",(struct IBox *)&pos,data->window->RPort->TxHeight);
	}
...
short filetype_save(config_filetypes_data *data)
{
	...
	// Open a port to stop galileo re-reading
			...
			else
			{
				...
				if (!list->path[0])
				{
					...
					if (list->flags&FTLISTF_STORE)
						strcpy(list->path,"PROGDIR:Filetypes/Storage/");
					else
						strcpy(list->path,"PROGDIR:Filetypes/");
				...
				if (list->flags&FTLISTF_STORE)
				{
					...
					lsprintf(buf,"PROGDIR:Storage/Filetypes/%s",FilePart(list->path));

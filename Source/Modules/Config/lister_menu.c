	...
	register __d3 Att_List *script_list)
{
	...
	// Initial dimensions
	data->pos_valid=LoadPos((type==TYPE_HOTKEYS)?"Galileo/windows/hotkeys":"Galileo/windows/scripts",&data->window_pos,&data->font_size);
...
void lister_menu_close(lister_menu_data *data,BOOL free)
{
	if (data)
	{
		...
		{
			SavePos((data->type==TYPE_HOTKEYS)?"Galileo/windows/hotkeys":"Galileo/windows/scripts",&data->window_pos,data->font_size);
		}
	...
	FunctionReturn *ret)
{
		...
		(button=(Cfg_Button *)node->data))
	{
		...
		if (!(func=NewButtonFunction(data->bank->memory,FTYPE_LEFT_BUTTON))) // Plug memory leak
			return 0;
	...
	unsigned short gadgetid)
{
	...
	do
	{
		...
		if (gadgetid==MENU_LISTER_MENU_SAVEAS || !path[0])
		{
			...
			// Invalid path?
			if (!path[0]) strcpy(path,"PROGDIR:buttons/");
...
void scripts_check_bank(lister_menu_data *data)
{
	...
	// Default name
	strcpy(data->menu_name,"PROGDIR:buttons/scripts");
}

...
/*
 *	Check all Galileo filetype files in dir for filetype matching file.
 */
...
int finder_save_edited_filetype( finder_data *data )
{
	...
	if (ftl = AllocMemH( 0, sizeof(Cfg_FiletypeList) ))
	{
	    ...
	    if (stricmp( data->best_installed_ft->list->path, data->edited_filetype->type.name ) == 0
		    ...
		    "PROGDIR:Filetypes", data->edited_filetype->type.name ))
	    {
		/* 5/8/97 (JP) : Added path part */
		strcpy( ftl->path,"PROGDIR:Filetypes/" );
...
int finder_build_cache( finder_data *data )
{
	...
	if (ok)
	{
	    if (lock = Lock( "PROGDIR:Storage/Filetypes", ACCESS_READ ))
	    {
...
int creator_warn_edited( creator_data *data )
{
    ...
    if (data->edited)
	ok = my_SimpleRequestTags(data->window,
...
// Init code for process
ULONG __saveds __asm creator_editor_proc_init(register __a0 IPCData *ipc,
...
void __saveds creator_editor_proc_code( void )
{
    ...
    if (data->filetype && (GalileoConfigBase = OpenLibrary( "PROGDIR:Modules/galileoconfig.gfmmodule", 0 )))
    {
...
int creator_save_filetype( creator_data *data )
{
	...
	if (data->filetype_list)
	{
	    if (!exists_req( data->window, GetString( locale, MSG_CREATE_TITLE ),
		    "PROGDIR:Filetypes", data->filetype->type.name ))
	    {
	...
	register __d1 CONST GalileoCallbackInfo *gci)
{
    ...
    if (newdir = Lock( "PROGDIR:Filetypes", ACCESS_READ ))
    {

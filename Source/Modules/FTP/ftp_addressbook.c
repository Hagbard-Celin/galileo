...
SavePos("Galileo/windows/ftp",(struct IBox *)&pos,window->RPort->TxHeight);
...
if	(node = Att_FindNode( dg->dg_og->og_SiteList, dg->dg_drag_item ))
	...
	if	(name = AllocVec( strlen(entry->se_name) + 1, MEMF_ANY ))
		...
		// Create the function
		if	(CreateFunctionFile( path, INST_COMMAND, command, "PROGDIR:Icons/FTPSite" ))
			{
			ok = 1;
...
switch	(wp->wp_option)
	...
	case ENV_SUB_COPYATTR:
		...
		SetGadgetValue(objlist,GAD_ENV_COPY_GALILEO_DEFAULT,env->e_copy_galileo_default);
		...
		DisableObject(objlist,GAD_ENV_COPY_DATESTAMP,env->e_copy_galileo_default);
		DisableObject(objlist,GAD_ENV_COPY_PROTECTION,env->e_copy_galileo_default);
		DisableObject(objlist,GAD_ENV_COPY_COMMENT,env->e_copy_galileo_default);
...
switch	(wp->wp_option)
	...
	case ENV_SUB_COPYATTR:
		...
		env->e_copy_galileo_default=GetGadgetValue(objlist,GAD_ENV_COPY_GALILEO_DEFAULT);
...
if	(cm && cm->cm_galileo) // Is this a valid message ?
	{
	struct site_entry *e;
	struct window_params *wp;
...
if	(am && am->am_galileo)
	...
	// Fill out galileo port - needed for sending connect messages
	// This implementation will break if running multiple Galileos
 ...
 * remove PROGDIR:system if selected path
...
if	(ok=AslRequestTags(DATA(wp->wp_win)->request,
	...
	if	(stricmp("PROGDIR:buttons",DATA(wp->wp_win)->request->fr_Drawer))
		{
...
*/
		...
		case GAD_ENV_COPY_GALILEO_DEFAULT:
			{
			int val=GetGadgetValue(wp->wp_sub_objlist,GAD_ENV_COPY_GALILEO_DEFAULT);
...
/*****************************************************************
 *
 *	Addressbook process started by main galileoftp

...
//
//	Add entry to a lister using Galileo callback hook
//
void lister_add(
...
//
//	Refresh a lister via Galileo callback hook or ARexx
//
...
// Ask Galileo for the info on this entry
if	(fileinfo = rexx_lst_query_entry( node->fn_galileo, node->fn_handle, entryname ))
	{
	retval = TRUE;
...
// Else show info in title bar
	...
	{
	oldtitle = rexx_lst_title_swap(ftpnode->fn_galileo,ftpnode->fn_handle,GetString(locale,MSG_FTP_CHANGING_DIR) );

	send_rexxa(ftpnode->fn_galileo,FALSE,"lister refresh %lu full",ftpnode->fn_handle );
	}
...
if	(progress)
	...
	{
	rexx_lst_title( ftpnode->fn_galileo, ftpnode->fn_handle, oldtitle );
...
if	(name)
	{
	rexx_lst_lock( ftpnode->fn_galileo, ftpnode->fn_handle );
	...
	if	(reply / 100 == COMPLETE)
		...
		if	(msg->flags & MKDIR_SELECT)
			rexx_lst_select( ftpnode->fn_galileo, ftpnode->fn_handle, name, 1 );
...
if	(fm->fm_names && *fm->fm_names && (namearray = alloc_name_array( fm->fm_names )))
	...
	total = rexx_lst_query_numselfiles( ftpnode->fn_galileo, handle );
	...
	for	(namep = namearray; *namep; namep++)
		...
		if	(gotfileinfo || used_req)
			...
			rexx_lst_label( ftpnode->fn_galileo, handle, 0, ei.ei_name, 0 );
			...
			if	(lister_request(
				...
				if	(reply / 100 == COMPLETE)
					...
					if	(!used_req || exactname)
						rexx_lst_remove( ftpnode->fn_galileo, handle, *namep );
	...
	rexx_lst_unlock( ftpnode->fn_galileo, handle );
...
if	(fm->fm_entries)
	...
	if	(source = create_endpoint_tags(
		...
		if	(confirm_commence)
			...
			for	(
				...
				if	(confirm_entry == 1 || confirm_entry == 2)
					...
					if	(success = recursive_delete( &hc, startdirname, ei ) == 2)
						{
						rexx_lst_remove( ftpnode->fn_galileo, ftpnode->fn_handle, ei->ei_name );
						rexx_result = 1;
						}
					}
			...
			rexx_lst_refresh( ftpnode->fn_galileo, ftpnode->fn_handle, 0 );
			}
...
if	(!strcmp( em->em_field, "name" ))
	...
	if	(ftp_rename( &ftpnode->fn_ftp, em->em_entry, em->em_value ) == 250)
		...
		if	(entry_info_from_lister( ftpnode, em->em_entry, &ei, 0 ))
			...
			rexx_lst_remove( ftpnode->fn_galileo, ftpnode->fn_handle, em->em_entry );
...
if	(sm = AllocVec( sizeof(struct connect_msg), MEMF_CLEAR ))
	...
	// Copy the Galileo lister's format into ours
...
if	(names = pm->pm_names)
	...
	if	(source = create_endpoint_tags(
		...
		// Multiple entries.  Query using callbacks.
			...
			{
			filecount = rexx_lst_query_numselfiles( ftpnode->fn_galileo, ftpnode->fn_handle );
			dircount  = rexx_lst_query_numseldirs( ftpnode->fn_galileo, ftpnode->fn_handle );
			}
...
while	(1) // for colin :)
	...
	if	(GetVar( "Galileo/NoBeeGees", &env, 1, GVF_GLOBAL_ONLY ) == -1)
		{
...
rexx_prog_bar( node->fn_galileo, handle, type, total, count );
}
...
rexx_prog_bytes( node->fn_galileo, handle, type, total, count );
}
...
rexx_prog_clear( node->fn_galileo, handle, type );
...
rexx_prog_info( node->fn_galileo, handle, type, info );
}
...
rexx_prog_info2( node->fn_galileo, handle, type, info2 );
...
rexx_prog_info3( node->fn_galileo, handle, type, info3 );
}
...
handle = rexx_prog_init( node->fn_galileo, handle, type, title, info, name, file, bar );
...
if	(short_display)
	handle = rexx_prog_init( node->fn_galileo, handle, type, title, "", name, file, bar );
else
	handle = rexx_prog_init3( node->fn_galileo, handle, type, title, "","","", name, file, bar );
...
rexx_prog_name( node->fn_galileo, handle, type, name );
}
...
if	(s = rexx_lst_query_path( node->fn_galileo, node->fn_handle ))
	{
	if	(buffer)
...
//
//	Structure copied from //Program/misc.h
//
struct read_startup
{
struct List *files;
...
if	(*filename)
	...
	if	(result = (char *)send_rexxa( ftpnode->fn_galileo, REXX_REPLY_RESULT, "galileo read %lu delete %s", ftpnode->fn_read_handle, filename ))
		{
...
//	from the Galileo lister's copy
//
...
//
//	Tell the Galileo lister to update its copy of the lister format
...
// Tell Galileo lister to update its format

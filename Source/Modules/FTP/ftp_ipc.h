...
IPC_TRAPTEMP,			// Issue an Galileo command that uses a temporary local file
...
char am_galileo[PORTNAMELEN+1];		// Instance of Galileo to call
};
...
// Trapped Galileo command that uses a temporary file
struct traptemp_msg
{
struct RexxMsg *tm_rxmsg;
char            tm_command[TRAPTEMPCMDLEN + 1];	// The Galileo command to send

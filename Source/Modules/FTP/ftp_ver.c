...
19.02.97
	...
	63.28	Changed site leftout icon to FTPSite.info
	  ...
	  .38	Implemented support for servers with broken ls command
			...
			Set Env:Galileo/ftp_broken_list for now.
	  ...
	  .41	Added environment variable "Galileo/ftp_fred_hack" which
	  		allows GalileoFTP to work with filenames which begin
...
19.3.99	68.5	Modified unix_line_to_entryinfo to account for foreign date strings
	...
	68.11	Fixed problems with lost connections 421 on passive and other
		modes. Galileo/NoBeeGees will disable check
	...
	68.13	Now preserves addressbook size via Galileo/windows/ftp

...
if	(ui = AllocVec( sizeof(struct update_info), MEMF_CLEAR ))
	...
	if	(!redo_cache)
		buffered = rexx_lst_findcache( ftpnode->fn_galileo, handle, ftpnode->fn_site.se_path );

	if	(!buffered || redo_cache)
		{
		// No need to do an empty if entry is cached, use clear
		if	(redo_cache)
			rexx_lst_clear( ftpnode->fn_galileo, handle );
		else
			rexx_lst_empty( ftpnode->fn_galileo, handle ); 

		// We just cleared the path - put it back
		rexx_lst_set_path( ftpnode->fn_galileo, handle, ftpnode->fn_site.se_path );
...
/*gp changed to use flag and not use fred_hack env var */
		...
		if	(progress)
			...
			{
			rexx_lst_title( ftpnode->fn_galileo, handle, GetString(locale,MSG_READING_DIR) );
	...
	rexx_lst_title( ftpnode->fn_galileo, handle, ftpnode->fn_site.se_host );
	send_rexxa( ftpnode->fn_galileo, FALSE, "lister refresh %lu full", handle );

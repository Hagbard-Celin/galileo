...
og.og_galileo_ipc		= galileo_ipc;	// ** GJP IMPORTANT Must be set 
...
if	(am = AllocVec( sizeof(*am), MEMF_CLEAR ))
	...
	stccpy( am->am_galileo, og.og_galileoname, PORTNAMELEN + 1 );
...
if	(fa = ParseArgs( CONNECT_TEMPLATE, args ))
	...
	if	(cm = get_blank_connectmsg( &og ))
		...
		// When Galileo calls us for 'FTP://', the path has already been converted to DIR=xxx
		if	(fa->FA_Arguments[D_OPT_HOST]

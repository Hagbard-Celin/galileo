...
#define GALILEOFM_HOOK_COUNT	45
...
typedef struct
{
	...
	BOOL __asm (*gc_CheckAbort) (register __a0 APTR handle);


	// Get window pointer from a path handle
	struct Window *	(*gc_GetWindow) (register __a0 APTR path);


	// Get Galileo ARexx port name
	struct MsgPort *(*gc_GetPort)
				(register __a0 char *ame);


	// Get Galileo public screen name
	struct Screen *	(*gc_GetScreen)
	...
	void __asm	(*gc_Private1)(void);
	void __asm	(*gc_Private2)(void);

...
if	(tmpnode = AllocVec( sizeof(struct ftp_node), MEMF_CLEAR ))
	...
	if	(ok)
		send_rexxa( galileo, REXX_REPLY_RESULT, "lister set %lu handler '"PORTNAME"' leavegauge", lclhandle );
	}
...
if	(hc->hc_galileo && hc->hc_proghandle)
	{
	if	(dirname)
	...
	if	(srcent)
		...
		rexx_lst_label( hc->hc_galileo, hc->hc_proghandle, 0, srcent->ei_name, 0 );
		}
	}
...
rexx_lst_lock( remotenode->fn_galileo, remotenode->fn_handle );
if	(xm->xm_otherhandle)
	rexx_lst_lock( remotenode->fn_galileo, xm->xm_otherhandle );
else
...
if	(names = xm->xm_names)
	...
	if	(msg->command == IPC_GET)
		...
		// Dest lister?  Query its path
			...
			{
			if	(path = rexx_lst_query_path( remotenode->fn_galileo, xm->xm_otherhandle ))
				freepath = 1;
			}
		...
		if	(path = rexx_lst_query_path( remotenode->fn_galileo, remotenode->fn_handle ))
			{
...
//		else
		...
		// Get 'To' path for progress bar
		else if	(path = rexx_lst_query_path( remotenode->fn_galileo, remotenode->fn_handle ))
			{
			final_path( path, l->hc.hc_todirname );
			DeleteArgstring( path );
			}
	...
	if	(source && dest)
		...
		// Multiple entries.  Query using callbacks.
			...
			{
		//	filecount = rexx_lst_query_numselfiles( remotenode->fn_galileo, srchandle );
			dircount  = rexx_lst_query_numseldirs( remotenode->fn_galileo, srchandle );
			}
		...
		if	(confirm_commence)
			...
			l->hc.hc_galileo = remotenode->fn_galileo;
					...
					//if	(confirm_entry == ?)
						...
						if	(success = recursive_copy( &l->hc, l->startdirname, l->fulldirname, &ei, dest_list ))
							...
							if	(srchandle)
								{
								if	(success == 3 && xm->xm_flags & XFER_MOVE)
									rexx_lst_remove( remotenode->fn_galileo, srchandle, ei.ei_name );
								else if	(success == 2 && !(xm->xm_flags & XFER_MOVE))
									rexx_lst_select( remotenode->fn_galileo, srchandle, ei.ei_name, 0 );
								}
							}
						}
			...
			rexx_lst_refresh( remotenode->fn_galileo, desthandle, REFRESH_DATE );

			// Show updated contents in source
			if	(srchandle)
				rexx_lst_refresh( remotenode->fn_galileo, srchandle, REFRESH_NODATE );
			}
		}
...
if	(xm->xm_otherhandle)
	rexx_lst_unlock( remotenode->fn_galileo, xm->xm_otherhandle );
	...
	if	(gotfileinfo = entry_info_from_lister( node, fm->fm_names, &ei, 0 ))
		...
		if	(ei.ei_type >= 0)
			...
			if	(strcmp( ei.ei_name, "." ) && strcmp( ei.ei_name, ".." ))
				{
				// Lock lister
				rexx_lst_lock( node->fn_galileo, node->fn_handle );
				...
				rexx_lst_unlock( node->fn_galileo, node->fn_handle );
		...
		if	(ei.ei_type < 0 || (ei.ei_type == 0 && !cwdok && node->fn_ftp.fi_reply != 421))
			...
			rexx_lst_lock( node->fn_galileo, handle );
			...
			rexx_lst_label( node->fn_galileo, handle, 0, ei.ei_name, 0 );
			...
			if	(rexx_result)
				rexx_doubleclick( node->fn_galileo, temp );
			...
			rexx_lst_unlock( node->fn_galileo, handle );
			}
...
//
//	Do an Galileo command on some files eg 'SmartRead'
//
...
if	(node && msg && tm)
	...
	if	(names = tm->tm_names)
		...
		if	(total = rexx_lst_query_numselfiles( node->fn_galileo, handle ))
			{
			lister_prog_init_multi(
				node,
				GetString(locale,MSG_FTP_RETRIEVING_FILE),
			...
			for	(p = names + 1; !(node->fn_flags & LST_ABORT); p += 3)
				...
				if	(ei.ei_type <= 0 && gotfileinfo)
					...
					if	(rexx_result)
						{
						send_rexxa( node->fn_galileo, REXX_REPLY_NONE, "command %s %s", tm->tm_command, temp );
						didit = TRUE;
						}
					...
					if	(didit)
						{
						rexx_lst_select( node->fn_galileo, handle, name, 0 );
...
rexx_lst_lock( ftpnode->fn_galileo, handle );
...
rexx_lst_label( ftpnode->fn_galileo, handle, 0, remotename, 0 );
...
// lst_unlock( ftpnode->fn_galileo, handle );
...
rexx_lst_lock( thisnode->fn_galileo, thisnode->fn_handle );
rexx_lst_lock( thisnode->fn_galileo, xm->xm_rm_dest->fn_handle );
...
if	(names = xm->xm_names)
	...
	if	(source && dest)
		...
		// Get 'From' path for progress bar
		if	(path = rexx_lst_query_path( thisnode->fn_galileo, srchandle ))
			{
		...
		// Get 'To' path for progress bar
		if	(path = rexx_lst_query_path( thisnode->fn_galileo, desthandle ))
			{
		...
		// Multiple entries.  Query using callbacks.
			...
			{
		//	filecount = rexx_lst_query_numselfiles( thisnode->fn_galileo, srchandle );
			dircount  = rexx_lst_query_numseldirs( thisnode->fn_galileo, srchandle );
			}
		...
		if	(confirm_commence)
			...
			hc.hc_galileo = thisnode->fn_galileo;
			...
			//if	(GetVar( "Galileo/ftp_broken_list", &env, 1, 0 ) != -1)
				...
				if	(confirm_entry && entry_info_from_lister(srcnode,srcname,&ei,ENTRYFROMF_DEFAULT ))
					...
					//if	(confirm_entry == ?)
						...
						if	(success = recursive_copy( &hc, startdirname, fulldirname, &ei, 0 ))
							...
							if	(!(xm->xm_flags & XFER_SUBDIR))
								rexx_lst_add( thisnode->fn_galileo, desthandle,
							...
							if	(srchandle)
								{
								if	(success == 3 && xm->xm_flags & XFER_MOVE)
									rexx_lst_remove( thisnode->fn_galileo, srchandle, ei.ei_name );
								else if	(success == 2 && !(xm->xm_flags & XFER_MOVE))
									rexx_lst_select( thisnode->fn_galileo, srchandle, ei.ei_name, 0 );
								}
							}
						}
					//else if	(confirm_entry == 0)
			...
			// Show updated contents in destination
			//rexx_lst_refresh( thisnode->fn_galileo, desthandle, REFRESH_DATE );
...
// Make listers non-busy
rexx_lst_unlock( thisnode->fn_galileo, thisnode->fn_handle );

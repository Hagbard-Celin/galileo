...
//
//  display a message using Galileo AsyncReq call
//
...
//
//	Converts a string of quoted names (as passed from Galileo) to an array of strings
//
...
if	(GetVar( "Galileo/ftp_fred_hack", &env, 1, 0 ) != -1)
...
BOOL CreateFunctionFile(char *name,short type,char *instruction,char *icon)
{
	...
	if	(icon)
		...
		if	(!(obj = GetDiskObject( icon )))
			obj = GetDiskObject("PROGDIR:Icons/Command");

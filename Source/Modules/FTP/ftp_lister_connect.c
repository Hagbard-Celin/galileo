...
// Lister busy
rexx_lst_lock( cld->cld_cm->cm_galileo, cld->cld_handle );
...
if	(logged_in)
	...
	if	(reply != 421 && ftp_image( &node->fn_ftp ) != 421)
		...
		rexx_lst_set_path( node->fn_galileo, node->fn_handle, node->fn_site.se_path );

		lister_prog_clear( node );
		rexx_lst_unlock( cld->cld_cm->cm_galileo, cld->cld_handle );
		...
		****************************************************/
		...
		send_rexxa( cld->cld_cm->cm_galileo, REXX_REPLY_NONE, "lister refresh %lu full", cld->cld_handle );
		rexx_lst_label( cld->cld_cm->cm_galileo, cld->cld_handle, "FTP:", cld->cld_cm->cm_site.se_host, NULL );
...
if	(!cld->cld_okay)
	...
	// Remove handler
	send_rexxa( cld->cld_cm->cm_galileo, REXX_REPLY_NONE, "lister set %lu handler ''", cld->cld_handle );
	}
...
if	(imsg && (cm = imsg->data_free))
	...
	if	(cm->cm_galileo && *cm->cm_galileo && lister_get_args( ogp, mld, imsg ))
		...
		if	(node = lister_create_node( ogp, mld->mld_ipc, cm ))
			...
			if	(handle = rexx_lst_new(cm->cm_galileo,cm->cm_handle,
						cm->cm_site.se_host,
				...
				if	(!rexx_lst_query_visible( cm->cm_galileo, handle ))
...
#ifdef DEBUG
kprintf( "galileo_lostconn() - %ld  ",ftpnode->fn_ftp.fi_errno );
...
strcpy( galileo, ftpnode->fn_galileo );
...
rexx_lst_unlock( galileo, ftpnode->fn_handle );
...
if	(ftpnode->fn_flags & LST_OPENED)
	...
	if	((ftpnode->fn_flags & LST_LEAVEOPEN)
		...
		send_rexxa( galileo, REXX_REPLY_NONE, "lister freecaches %lu "PORTNAME"", ftpnode->fn_handle);

		// Remove handler so we don't get 'inactive' msg 
		send_rexxa( galileo, REXX_REPLY_NONE, "lister set %lu handler ''", ftpnode->fn_handle );

		send_rexxa( galileo, REXX_REPLY_NONE, "lister set %lu source", ftpnode->fn_handle );

		// No command to issue after quitting?
		if	(!mld->mld_quitmsg || !mld->mld_quitmsg->flags)
			rexx_lst_refresh( galileo, ftpnode->fn_handle, REFRESH_NODATE );
		}
	...
	// Otherwise close lister
		...
		// Now close it
		rexx_lst_close( galileo, ftpnode->fn_handle );
		}
	}
...
if	(mld->mld_quitmsg && (qm = (struct quit_msg *)mld->mld_quitmsg->flags))
	...
	if	(qm->qm_command)
		send_rexxa( galileo, REXX_REPLY_NONE, "command %s", qm->qm_command );
	}
...
// Reconnect command?
	...
	if	(mld->mld_reconnecting)
		send_rexxa( galileo, REXX_REPLY_NONE, "command %s", mld->mld_quit_command );

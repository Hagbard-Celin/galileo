...
/// ChangeDir
void ChangeDir(struct ModuleData *data, struct Tree *cur)
{
    struct Tree *tmp;
    struct List list;
    UWORD dc=0, fc=0;
...
/// MsgPort
BOOL AllocPort(struct ModuleData *data)
{
	...
	sprintf(data->mp_name,"XADbrowser_%p",data->mp);
	AddPort(data->mp);
...
///
/// Copy
void _copy(struct ModuleData *data,char *name, char *Dest, BOOL CopyAs)
{
    struct Tree *tmp=data->cur->Child;
    struct xadFileInfo *xfi;
    ...
    data->ptr=OpenProgressWindowTags(PW_Window,	     data->listw,
...
///
/// Path
BOOL _path(struct ModuleData *data, char *path)
{
...
/// Extract Archive
BOOL ExtractD(struct ModuleData *data)
{
    struct DosList *dl;
    struct DosEnvec *de;
...
/// Main Extract
	...
	if (data.ArcInf=xadAllocObject(XADOBJ_ARCHIVEINFO,NULL))
	{
		...
		if ((!err) && (data.ArcMode == XADCF_DISKARCHIVER))
		{
		    // *** DISKARCHIVE
		    struct TagItem ti[2];

	...
	struct ListLock *main_list)
{
	...
	error=MatchFirst("PROGDIR:filetypes/~(#?.info)",anchor);
	...
	struct ListLock *main_list)
{
	...
	if (list=AllocMemH(memory,sizeof(Cfg_FiletypeList)))
	{
		...
		if (type=filetype_default_new(memory,list,"Galileo Start Menu","STRTMEN",30,"loadbuttons start","PROGDIR:icons/startmenu.info"))
		{
			// Recognition string for buttons
			lsprintf(type->recognition,"%lcGILO%lc%lcBTNW%lc%lcSTRT%lc%lcBPIC%lc%lcSTRT",
				FTOP_MATCHFORM,
		...
		if (type=filetype_default_new(memory,list,"Galileo Buttons","BUTTONS",12,"loadbuttons","PROGDIR:icons/buttons.info"))
		{
			// Recognition string for buttons
			lsprintf(type->recognition,"%lcGILO%lc%lcBTNW",FTOP_MATCHFORM,FTOP_AND,FTOP_MATCHCHUNK);
		}

		// Create default environment filetype
		if (type=filetype_default_new(memory,list,"Galileo Environment","ENVIRON",12,"loadenvironment","PROGDIR:icons/environment.info"))
		{
			// Recognition string for environment
			lsprintf(type->recognition,"%lcGILO%lc%lcENVR",FTOP_MATCHFORM,FTOP_AND,FTOP_MATCHCHUNK);
		}

		// Create default filetype filetype
		if (type=filetype_default_new(memory,list,"Galileo Filetype","FTYPE",12,"editfiletype","PROGDIR:icons/filetype.info"))
		{
			// Recognition string for filetypes
			lsprintf(type->recognition,"%lcGILO%lc%lcTYPE",FTOP_MATCHFORM,FTOP_AND,FTOP_MATCHCHUNK);
		}

		// Create default command filetype
		if (type=filetype_default_new(memory,list,"Galileo Command","CMD",16,"runcommand","PROGDIR:icons/command.info"))
		{
			// Recognition string for command
			lsprintf(type->recognition,"%lcGILO%lc%lc12%lc%lcFUNC",FTOP_MATCHFORM,FTOP_AND,FTOP_MOVE,FTOP_ENDSECTION,FTOP_MATCH);
		...
		if (type=filetype_default_new(memory,list,"Galileo Module","MODULE",21,0,"PROGDIR:icons/module.info"))
		{
			// Recognition string for modules
			lsprintf(type->recognition,"%lc*.gfmmodule%lc%lc$000003f3",FTOP_MATCHNAME,FTOP_AND,FTOP_MATCH);
		}

		// Create default rexx module filetype (no function, recognition only)
		if (type=filetype_default_new(memory,list,"Galileo Module","MODULE",14,0,"PROGDIR:icons/module.info"))
		{
			// Recognition string for modules
			lsprintf(type->recognition,"%lc*.galileo%lc%lc/*",FTOP_MATCHNAME,FTOP_AND,FTOP_MATCH);
		}

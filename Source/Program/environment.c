...
environment_save(Cfg_Environment *env,char *name,short snapshot,CFG_ENVR *data)
{
	...
	while (iff=IFFOpen(name,MODE_NEWFILE,ID_GILO))
	{
		IPCData *ipc;
		Lister *lister;
		Buttons *buttons;
	...
	{
		WriteFileIcon("PROGDIR:icons/Environment",name);
	}
...
void env_update_pathlist()
{
	...
	if (environment->env->env_flags&ENVF_USE_PATHLIST)
	{
		...
		for (num=0,path="env:Galileo/paths";num<2;num++,path="envarc:Galileo/paths")
		{
	...
	{
		DeleteFile("env:Galileo/paths");
		DeleteFile("envarc:Galileo/paths");
	}
...
void env_fix_themes()
{
	...
	if (!environment->env->themes_location[0])
		strcpy(environment->env->themes_location,"PROGDIR:Themes/");

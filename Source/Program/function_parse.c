...
function_parse_function(FunctionHandle *handle)
{
	...
	if (user_depth<MAXPARSEDEPTH)
	{
		...
		if (command->flags&FUNCF_EXTERNAL_FUNCTION &&
		    strnicmp(command->stuff.module_name,"PROGDIR:commands/",17)==0)
		{
		    // Load the function
...
void function_parse_instruction(FunctionHandle *handle,
	                        char *string,
...
function_build_instruction(FunctionHandle *handle,
...
function_add_filename(char *func_string,
	              char *filename1,
	              char *filename2,
	              int no_quote,
	              short slash)
...
void function_parse_arguments(FunctionHandle *handle,
	                      InstructionParsed *ins)
{
    char *ptr,*buf;
    short arg,limit;

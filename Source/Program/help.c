...
void help_show_help(char *thing,char *file)
{
	...
	if (!(ipc=IPC_FindProc(&GUI->process_list,"galileo_help",0,0)))
	{
		// Unlock process list
	...
	// No file given?
	if (!file || !*file) file="PROGDIR:Help/galileofm.guide";
...
void __saveds help_proc(void)
{
	...
	if (ipc=IPC_ProcStartup(0,0))
	{
		...
		strcpy(filename,"PROGDIR:Help/galileofm.guide");
...
AMIGAGUIDECONTEXT help_open_database(struct NewAmigaGuide *guide,char *name,struct Screen *screen,char *screen_name)
{
	...
	guide->nag_ClientPort="GALILEOFM_HELP";

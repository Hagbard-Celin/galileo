...
function_open_script(FunctionHandle *handle)
{
	...
	{
		// Build script name
		lsprintf(handle->script_name,"T:Galileo-%lx-tmp",handle->temp_key+a);
		...
		if (run_okay)
		{
			...
			if (handle->func_parameters.flags&FUNCF_OUTPUT_FILE)
			{
				...
				else
				{
					...
					lsprintf(handle->temp_name,"T:Galileo-%lx-out",handle->temp_key);
						...
						if (list=Att_NewList(0))
						{
							...
							if (node=Att_NewNode(list,handle->temp_name,0,0))
							{
								...
								if (startup=AllocVec(sizeof(struct read_startup),MEMF_CLEAR))
								{
									...
									if (misc_startup("galileo_read",FUNC_READ,0,startup,FALSE))
/* */									return;
									FreeVec(startup);
								}
							}

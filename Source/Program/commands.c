...
void init_commands_scan(short type)
{
	...
	if (type==SCAN_BOTH)
		ptr="PROGDIR:modules/#?.(gfmmodule|galileo)";
	else
	if (type==SCAN_MODULES)
		ptr="PROGDIR:modules/#?.gfmmodule";
	else
	if (type==SCAN_USER)
		ptr="PROGDIR:commands/~(#?.info)";
	else ptr="PROGDIR:modules/#?.galileo";
	...
	while (!error)
	{
		...
		if (sufcmp(anchor->ap_Info.fib_FileName,".gfmmodule"))
		{
			// Set flag
			real_module=1;
			...
			if (ModuleBase=OpenLibrary(anchor->ap_Buf,0))
			{
				...
				if (info=Module_Identify(-1))
				{
					...
					if (info->flags&MODULEF_CALL_STARTUP)
					{
						...
						if (misc_startup("galileo_module_init",MODULE_STARTUP,0,ModuleBase,0))
							ModuleBase=0;
					}
				}
		...
		node->node.ln_Succ;)
	{
		...
		if (!(node->flags&MNF_TEMP))
		{
			...
			if ((sufcmp(node->node.ln_Name,".galileo") && type&SCAN_REXX) ||
				(sufcmp(node->node.ln_Name,".gfmmodule") && type&SCAN_MODULES))
			{
				// Remove node
...
void command_new(BackdropInfo *info,IPCData *ipc,char *filename)
{
	...
	{
		// Can't open?
		if (!(iff=IFFOpen(filename,IFF_READ,ID_GILO)))
			return;
	...
	if (func)
	{
				...
				GetString(&locale,MSG_CANCEL),0))
		{
			...
			while (!iff)
			{
				...
				// Try to open file
				if (!(iff=IFFOpen(buffer,IFF_WRITE,ID_GILO)))
				{

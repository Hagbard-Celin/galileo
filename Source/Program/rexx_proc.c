...
void __saveds rexx_proc(void)
{
    ...
    KPrintF("rexx_proc ResTrackBase: %lx \n",ResTrackBase);
#endif
	...
	if (handle=function_new_handle(0,1))
	{
		...
		notify_req=AddNotifyRequest(GN_DOS_ACTION,0,notify_port);
	...
	FOREVER
	{
		...
		while (nmsg=(GalileoNotify *)GetMsg(notify_port))
		{
			// Handle notify
...
BOOL rexx_process_msg(struct RexxMsg *msg,struct MsgPort *reply,long *count)
{
	...
	switch (command)
	{
		...
		case RXCMD_GALILEO:
			...
			// Handle command
			ret=rexx_galileo_cmd(msg,subcommand,commandptr);
			break;
	}
...
RexxReader *rexx_read_file(short command,char *args,struct RexxMsg *msg)
{
	...
	if (!reader && !quit)
	{
			...
			(node=Att_NewNode((Att_List *)startup->files,args,0,0)))
		{
			...
			// Launch reader
			if (!(reader->ipc=misc_startup("galileo_rexx_reader",command,0,startup,FALSE)))
			{
...
BOOL rexx_send_msg(struct MinList *list,IPCMessage **imsg,struct MsgPort *port)
{
	...
	// Create message
	if (!(msg=CreateRexxMsg(port,".galileo",port->mp_Node.ln_Name)))
		return 0;

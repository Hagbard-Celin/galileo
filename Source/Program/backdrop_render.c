	...
	short top)
{
	...
	if (flags&BRENDERF_REAL && !(object->bdo_flags&BDOF_NO_LABEL))
	{
		...
		if (len=strlen(name))
		{
			...
			texty=rect.MaxY+ICON_LABEL_SPACE+rp->TxBaseline+((galileo_drawmode==MODE_OUTLINE)?1:0);
			Move(rp,textx,texty);
			...
			if (galileo_drawmode==MODE_SHADOW)
			{
			...
			if (galileo_drawmode==MODE_OUTLINE)
			{
			...
			{
				EraseRect(rp,
					extent.te_Extent.MinX,rect.MaxY+ICON_LABEL_SPACE+((galileo_drawmode==MODE_OUTLINE)?1:0),

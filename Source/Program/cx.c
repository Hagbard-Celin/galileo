					...
					if (count==id)
					{
						...
						if (GUI->flags2&GUIF2_KEY_FINDER)
						{
							...
							if (GetWindowID((window=IntuitionBase->ActiveWindow))==WINDOW_KEYFINDER)
							{
								...
								if (ipc=IPC_FindProc(&GUI->process_list,"galileo_keyfinder",TRUE,0))
								{
...
void __saveds cx_right_button(register CxMsg *cxm,CxObj *co)
{
	...
	if ((ie->ie_Class==IECLASS_RAWMOUSE || ie->ie_Class==IECLASS_POINTERPOS) &&
		GUI->screen_pointer)
	{
		// Mouse button push?
		if (ie->ie_Class==IECLASS_RAWMOUSE && !(ie->ie_Code&IECODE_UP_PREFIX) && !data->right_down)
		{
			struct Screen *scr;
			ULONG ilock;
			short which;
	...
	if (window)
	{
		...
		if (!(window->Flags&WFLG_WINDOWACTIVE))
		{
			...
			if (msg=AllocVec(sizeof(struct IntuiMessage),MEMF_CLEAR))
			{
				...
				if (ie->ie_Class==IECLASS_RAWMOUSE && ie->ie_Code==data->right_down)
				{
					struct Layer *layer;
					BOOL ok=1;

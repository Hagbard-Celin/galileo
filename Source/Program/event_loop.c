				...
				switch (notify->nm_NReq->nr_UserData)
				{
					...
					case NOTIFY_MODULES_CHANGED:
						...
						if (!(GUI->flags&GUIF_NO_NOTIFY))
						{
							...
							misc_startup("galileo_module_sniffer",MAIN_SNIFF_MODULES,GUI->window,(APTR)SNIFF_BOTH,1);
						}
						break;
					...
					case NOTIFY_ENV_CHANGED:
						...
						misc_startup("galileo_env_rooter",MAIN_ENV_UPDATE,GUI->window,0,1);
						break;
					...
					case NOTIFY_COMMANDS_CHANGED:
						...
						// Launch process to update modules
						misc_startup("galileo_command_sniffer",MAIN_SNIFF_MODULES,GUI->window,(APTR)SNIFF_USER,1);
						break;
				}
			}
...
*/
		...
		while (ipc_msg=(IPCMessage *)GetMsg(main_ipc.command_port))
		{
			...
			switch (ipc_msg->command)
			{
				...
				case MAINCMD_REFRESH_DRIVES:
					...
					// Update device list
					misc_startup("galileo_refresh_drives",REFRESH_MAIN_DRIVES,0,0,0);
					break;
		...
		if (GUI->backdrop->notify_req)
		{
			...
			// Get notify message	
			while (notify=(GalileoNotify *)GetMsg(GUI->backdrop->notify_port))
		...
		if (GUI->notify_req)
		{
			...
			// Get notify message
			while (notify=(GalileoNotify *)GetMsg(GUI->notify_port))
			{
				// AppMenu?
				if (notify->gn_Type==GN_APP_MENU_LIST &&
				...
				if (notify->gn_Type==GN_APP_ICON_LIST && notify->gn_Data)
				{
				...
				if (notify->gn_Type==GN_CLOSE_WORKBENCH)
				{
					// Is Galileo on the Workbench screen?
						...
						strcmp(get_our_pubscreen(),"Workbench")==0)
					{
						...
						// Launch process to shut Galileo down
						misc_startup("galileo_closewb",MAIN_CLOSEWB,GUI->window,0,TRUE);
					}
				}

				// Open Workbench?
				else
				if (notify->gn_Type==GN_OPEN_WORKBENCH)
				{
					// Close pending?
					if (GUI->flags&GUIF_CLOSE_PENDING)
					{
				...
				if (notify->gn_Type==GN_DISKCHANGE && GUI->window)
				{
					// Launch process to handle this
					misc_startup("galileo_disk_change",MAIN_DISK_CHANGE,GUI->window,notify,0);	// ** not exclusive
					notify=0;
				}

				// REXX started up
				else
				if (notify->gn_Type==GN_REXX_UP)
				{
					...
					if (!(GUI->flags&GUIF_REXX))
					{
						...
						misc_startup("galileo_module_sniffer",MAIN_SNIFF_MODULES,GUI->window,SNIFF_REXX,0);
					}
				}

				// Flush memory
				else
				if (notify->gn_Type==GN_FLUSH_MEM)
				{
					// Flush buffers
					buffer_mem_handler(notify->gn_Flags);
				}
				...
				if (backdrop_idcmp(GUI->backdrop,&msg_copy,BIDCMPF_DOUBLECLICK))
				{
					...
					else
					{
						...
						while (!(IsListEmpty((struct List *)&notify_list)))
						{
							...
							// Get message
							notify=(GalileoNotify *)notify_list.mlh_Head;
		...
		if (res&(1<<drag_timer->port->mp_SigBit))
		{
			...
			if (seed_timer && CheckTimer(seed_timer))
			{
				...
				if (file=Open("PROGDIR:system/seed",MODE_NEWFILE))
				{
			...
			if (CheckTimer(filetype_timer))
			{
				...
				else misc_startup("galileo_filetypes",MENU_NEW_FILETYPES,0,0,1);
			}
			...
			if (CheckTimer(delete_timer))
			{
				...
				if (copy=AllocVec(sizeof(struct DateStamp),0))
				{
					// Launch deleter
					*copy=last_janitor;
					misc_startup("galileo_janitor",DELETE_TEMP_FILES,0,copy,1);
				}
	...
	struct Window *window)
{
	...
	switch (id)
	{
		...
		case MENU_EXECUTE:
			{
				misc_startup("galileo_execute",MENU_EXECUTE,window,0,1);
			}
			break;
		...
		case MENU_ABOUT:
			{
				misc_startup("galileo_about",MENU_ABOUT,window,0,1);
			}
			break;
		...
		// Key Finder
		case MENU_KEYFINDER:
			misc_startup("galileo_keyfinder",MENU_KEYFINDER,window,0,1);
			break;
		...
		case MENU_QUIT:
			{
				if (environment->env->settings.general_flags&GENERALF_QUICK_QUIT)
					return 1;
				if (!(misc_startup("galileo_quit",MENU_QUIT,window,0,1)))
					return 1;
			}
			break;
		...
		case MENU_OPEN_STARTMENU:
			misc_startup("galileo_new_startmenu",id,window,0,0);
			break;
		...
		case MENU_SAVE_LAYOUT:
			{
				...
				if (IPC_FindProc(&GUI->process_list,"galileo_environment",TRUE,0))
					break;
		...
		case MENU_FILETYPES:
			{
				...
				misc_startup("galileo_config_filetypes",MENU_FILETYPES,window,0,1);
			}
			break;
		...
		case MENU_LISTER_MENU:
			misc_startup("galileo_config_lister_menu",MENU_LISTER_MENU,window,0,1);
			break;


		// Edit user menu
		case MENU_MENU:
			misc_startup("galileo_config_user_menu",MENU_MENU,window,0,1);
			break;


		// Edit hotkeys
		case MENU_HOTKEYS:
			misc_startup("galileo_config_hotkeys",MENU_HOTKEYS,window,0,1);
			break;


		// Edit scripts
		case MENU_SCRIPTS:
			misc_startup("galileo_config_scripts",MENU_SCRIPTS,window,0,1);
			break;
...
/*
		// Edit menu keys
		case MENU_MENUKEYS:
			misc_startup("galileo_config_menukeys",MENU_MENUKEYS,window,0,1);
			break;
*/
		...
		case MENU_ICON_DELETE:
			{
				...
				if (packet=get_icon_packet(GUI->backdrop,0,0,0))
				{
					// Start process
					if (!(misc_startup(
						(id==MENU_ICON_RENAME)?"galileo_icon_rename":"galileo_icon_delete",
						id,
						window,
		...
		// New group
		case MENU_GROUP_NEW:
			misc_startup("galileo_new_group",MENU_GROUP_NEW,window,GUI->backdrop,1);
			break;


		// New command
		case MENU_COMMAND_NEW:
			misc_startup("galileo_new_command",MENU_COMMAND_NEW,window,GUI->backdrop,1);
			break;
		...
		// Icon positioning
		case MENU_ICONPOS:
			misc_startup("galileo_iconpos",MENU_ICONPOS,GUI->window,GUI->iconpos_port,TRUE);
			break;
...
void handle_icon_notify(GalileoNotify *notify)
{
	// Adding an AppIcon?
	if (!(notify->gn_Flags&GNF_ICON_REMOVED))
	{
		AppEntry *icon=(AppEntry *)notify->gn_Data;
			...
			icon->ae_flags&APPENTF_LOCAL)
		{
			...
			// Change?
			if (notify->gn_Flags&GNF_ICON_CHANGED)
			{
				// Change flag
				flags|=BDAF_CHANGE;
				if (notify->gn_Flags&GNF_ICON_IMAGE_CHANGED)
	...
	{
		// Remove icon
		backdrop_rem_appicon(GUI->backdrop,(AppEntry *)notify->gn_Data);
	}
}

...
// Fill out a GalileoScreenData structure
...
// Check that a screen can close if Galileo disappears
BOOL check_closescreen(struct Screen *screen)
{
	...
	for (a=0;a<4;a++)
	{
			...
			window=window->NextWindow)
		{
			...
			{
				// If this is one of Galileo's, GetWindowID() should know about it
		...
		if (read=AllocVec(sizeof(struct read_startup),MEMF_CLEAR))
		{
			...
			if (read->files=(struct List *)Att_NewList(0))
			{
				...
				if (Att_NewNode((Att_List *)read->files,name,0,0))
				{
					...
					misc_startup("galileo_more_trap",FUNC_SMARTREAD,0,read,FALSE);
					ok=1;
				}
...
void handle_diskchange(GalileoNotify *notify)
{
	...
	{
		// Launch "bad" script
		RunScript(SCRIPT_BAD_DISKINSERT,notify->gn_Name);
		return;
	}
	else

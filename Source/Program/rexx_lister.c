				...
				switch (id)
				{
					...
					case RXCMD_LABEL:
						...
						if (lister->appicon)
						{
							...
							// Update app icon
							SendNotifyMsg(GN_APP_ICON_LIST,(ULONG)lister->appicon,GNF_ICON_CHANGED,FALSE,0,0);
						}
						break;
							...
							switch (id)
							{
								...
								case RXCMD_NAME:
									...
									if (copy=AllocMemH(0,strlen(args)+1))
									{
										...
										KPrintF("rexx_lister.c:1236 AllocMem: %lx \n", (copy)-2 );
#endif
...
#ifdef _DEBUG_ALLOCMEMH
				KPrintF("rexx_lister.c:2475 AllocMem: %lx \n", ((ULONG *)name)-2 );
#endif

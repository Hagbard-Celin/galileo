	...
	register __a6 struct Library *exec)
{
	...
	// Open library
	if (!(libbase=(struct MyLibrary *)FindName(&((struct ExecBase *)exec)->LibList,"galileofm.library")))
		return 0;
	...
	if (a>=16)
	{
		...
		if (strcmp(name+a-16,"galileo.datatype")==0)
			name="galileofm.library";
	}
	...
	register __a6 struct MyLibrary *libbase)
{
	...
	header->dth_Name="galileo";
	header->dth_BaseName="galileo";

...
// Listview dispatcher
ULONG __asm __saveds listview_dispatch(register __a0 Class *cl,
	                               ...
	                               register __a1 Msg msg)
{
	...
	case OM_SET:
		...
		if (tag=FindTagItem(GLV_ScrollUp,tags))
		{
		    ...
		    if (msg->MethodID==OM_UPDATE && (data->scroll_count==1 || data->scroll_count==2))
		    {
			// Throw this away
			++data->scroll_count;
		    }
...
short listview_get_sel(Class *cl,
	               struct Gadget *gadget,
	               ListViewData *data,
	               struct gpInput *input,
	               short outside_ok)

...
static char *RexxMsgIdentifier="GALILEO";
	...
	register __d0 UBYTE *host)
{
	...
	// Turn message into a special Galileo one
	...
	register __d1 long length)
{
	...
	if (IsRexxMsg(msg))
	{
		...
		return SetRexxVar(msg,varname,value,length);
	}

	// Check for valid Galileo ARexx message
	...
	register __a2 char **bufpointer)
{
	...
	// Standard ARexx message?
	if (IsRexxMsg(msg)) return GetRexxVar(msg,varname,bufpointer);

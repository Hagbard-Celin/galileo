	...
	register __a5 ULONG *switch_flags)
{
	...
	struct IBox pos;
	short fontsize;
	char *size_name="Galileo/windows/selection",namebuf[80];
	...
	if (flags&SLF_SWITCHES && switch_txt)
	{
		...
		// Different name
		lsprintf(namebuf,"Galileo/windows/selection%ld",switch_count);
		size_name=namebuf;
	}

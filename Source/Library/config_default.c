...
void __asm __saveds L_DefaultSettings(register __a0 CFG_SETS *settings)
{
	...
	settings->error_flags=ERROR_ENABLE_GALILEO_FM;
...
void __asm __saveds L_DefaultEnvironment(register __a0 CFG_ENVR *env)
{
	...
	strcpy(env->output_window,"20/10/600/180/Galileo Output");
	...
	strcpy(env->desktop_location,"Galileo:Desktop/");
...
*/
	...
	if (env->version<CONFIG_VERSION_11)
	{
		...
		if (GetVar("Galileo/PopUpDelay",buf,sizeof(buf),GVF_GLOBAL_ONLY)>0)
		{
		...
		if (GetVar("Galileo/CommandLineLength",buf,sizeof(buf),GVF_GLOBAL_ONLY)>0)
		{

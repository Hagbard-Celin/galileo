...
void main(int argc,char **argv)
{
	...
	// Need galileofm.library
	if (!(GalileoFMBase=OpenLibrary("Galileo:libs/galileofm.library",55)))
		exit(10);

	// Allocate data
	if (!(data=AllocMemH(0,sizeof(font_data))))
	{
		font_free(0);
		exit(5);
	}
...
BOOL font_open(font_data *data)
{
	...
	// Fill out new window
	data->new_win.nw_Parent=screen;
	data->new_win.nw_Dims=&data->win_dims;
	data->new_win.nw_Title=GalileoGetString(&data->locale,MSG_FONT_TITLE);
	data->new_win.nw_Locale=&data->locale;
	data->new_win.nw_Flags=WINDOW_SCREEN_PARENT|WINDOW_VISITOR|WINDOW_AUTO_KEYS|WINDOW_SIZE_BOTTOM|WINDOW_SIMPLE;
	...
	WindowLimits(data->window,
		(font_window.cw_CharDims.Width*data->window->RPort->TxWidth)+font_window.cw_FineDims.Width,
		(font_window.cw_CharDims.Height*data->window->RPort->TxHeight)+font_window.cw_FineDims.Height,
		~0,~0);
...
void font_ask_name(font_data *data)
{
	...
	if (AslRequestTags(((WindowData *)data->window->UserData)->request,
		...
		ASLFR_TitleText,GalileoGetString(&data->locale,MSG_FONT_SELECT),
...
void font_build_labels(font_data *data,struct FontContentsHeader *fch)
{
	...
	if (!(ptr=AllocMemH(0,(sizeof(char *)+8)*(num+1))))
		return;
	...
	for (a=0;a<num;a++,ptr+=8)
	{
		...
		{
			// Add size
			sprintf(labels[a],"%d",fc->fc_YSize);
...
void font_show_about(font_data *data)
{
	...
	easy.es_Flags=0;
	easy.es_Title=0;
	easy.es_TextFormat="Galileo Font Viewer\nv55.0 (01.08.96)\n\n© 1996 Jonathan Potter";
	easy.es_GadgetFormat=GalileoGetString(&data->locale,MSG_OK);
...
void font_save_settings(font_data *data)
{
	...
	// Set variable
	if (SetVar("Galileo/Font Viewer",buf,-1,GVF_GLOBAL_ONLY))
	{
		// Copy to ENVARC:
		SystemTagList("copy \"env:Galileo/Font Viewer\" envarc:Galileo",0);
	}
...
void font_read_settings(font_data *data)
{
	...
	if (GetVar("Galileo/Font Viewer",buf,sizeof(buf),GVF_GLOBAL_ONLY)<=0)
		return;
	...
	if (dims.Height>0)
	{
		...
		data->win_dims.cw_CharDims.Top=0;
		data->win_dims.cw_CharDims.Left=0;
		data->win_dims.cw_CharDims.Width=0;
		data->win_dims.cw_CharDims.Height=0;

		// Set absolute coordinates
		data->win_dims.cw_FineDims=dims;
	}
}

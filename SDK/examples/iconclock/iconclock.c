...
static char *version="$VER: iconclock.module 55.0 (15.08.96)";
	...
	register __d1 EXT_FUNC(func_callback))
{
	...
	// See if we're already running; return if we are
...
void __saveds icon_clock(void)
{
	...
	if (data->screen)
	{
		...
		// Initialise clock for this screen
		data->screen=0;
	...
	FOREVER
	{
		...
		{
			// We don't do anything with them at this stage
			IPC_Reply(msg);
		}

		// Notify messages from Galileo?
		while (nmsg=(GalileoNotify *)GetMsg(data->notify_port))
		{
			// Galileo has Quit?
			if (nmsg->gn_Type&GN_GALILEOFM_QUIT) quit=1;

			// Galileo has been hidden?
			else
			...
			// Galileo has been revealed
			else
			if (nmsg->gn_Type&GN_GALILEOFM_SHOW)
			{
				// Screen changed?
				if (data->screen!=((struct Window *)nmsg->gn_Data)->WScreen)
				{
					// Just made visible
					iconclock_init(data,((struct Window *)nmsg->gn_Data)->WScreen);
				}
				visible=1;
			}

			// Reply to the message
			ReplyFreeMsg((struct Message *)nmsg);
		}

		// About Requester open?
		if (data->about)
		{
			// Handle requester
...
// Startup code, called by IPC_ProcStartup()
	...
	register __a1 iconclock_data *data)
{
	...
	if (!(data->notify_req=
		AddNotifyRequest(
			GN_GALILEOFM_QUIT|GN_GALILEOFM_HIDE|GN_GALILEOFM_SHOW,
			0,
...
void iconclock_show(iconclock_data *data)
{
	...
	// Get colon we need (either upper or lower half flashing)
...
void iconclock_init(iconclock_data *data,struct Screen *screen)
{
	...
	if (FindPubScreen(screen,TRUE))
	{
		...
		// Under 39 grab a pen
		if (GfxBase->lib_Version>=39 && data->colour)
		{
			// Allocate pen
			...
			(UWORD *)AllocVec(RASSIZE(CLOCK_WIDTH,CLOCK_HEIGHT)*depth,MEMF_CHIP|MEMF_CLEAR))
		{
			...
			if (GfxBase->lib_Version>=39)
			{
				...
				tags[5].ti_Data=(ULONG)GalileoGetString(locale,MSG_ICONCLOCK_COLOUR);
...
void iconclock_info(iconclock_data *data)
{
	...
	easy.es_Flags=0;
	easy.es_Title=GalileoGetString(locale,MSG_ICONCLOCK_TITLE);
	easy.es_TextFormat="Galileo Icon Clock\nv55.0 (15.08.96)\n\n© 1996 Jonathan Potter";
	easy.es_GadgetFormat=GalileoGetString(locale,MSG_OK);
